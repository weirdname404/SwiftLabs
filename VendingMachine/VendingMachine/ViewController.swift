...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    let vendingMachine: VendingMachine // for flexibility
    ...
    @IBAction func purchase() {
        if let currentSelection = currentSelection {
            do {
                ...
                showAlertWith(title: "Insufficient Funds", message: "You still need \(required)$ to buy this \(currentSelection).")
            ...
            showAlertWith(title: "Nothing is selected!", message: "Please choose any item and try again.")
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if vendingMachine.item(forSelection: vendingMachine.selection[indexPath.row])?.quantity == 0 {
            updateCell(having: indexPath, selected: false, outOfStock: true)
        }
        else {
            ...
            // indexPath.row gives the index position we're tapping and we can use this index number
            // again to look in the selection array and figure out what object we're tapping
    ...
    func updateCell(having indexPath: IndexPath, selected: Bool, outOfStock: Bool) {
        ...
        if let cell = collectionView.cellForItem(at: indexPath) {
            ...
            if outOfStock {
                cell.contentView.backgroundColor = UIColor(red: 255/255.0, green: 102/255.0, blue: 102/255.0, alpha: 1.0)
            }
        }
    }
